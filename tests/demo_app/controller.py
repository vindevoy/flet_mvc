...
class DemoController(FletMVCController):
    def change_mode(self, e: ft.ControlEvent):
        ...
        self.app.change_theme_mode(ft.ThemeMode.DARK if e.control.value else ft.ThemeMode.LIGHT)
    ...
    def button_click(self, e: ft.ControlEvent = None):
        ...
        dlg = ft.AlertDialog(
            modal=True,
            actions=[
                ft.TextButton("Yes", on_click=self.app.close_application),
                ft.TextButton("No", on_click=self.app.close_dialog)
            ],
            actions_alignment=ft.MainAxisAlignment.END,
            title=ft.Text(f"Close the application ?")
        )

        self.app.open_dialog(dlg)
